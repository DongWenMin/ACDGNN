...
def sp_hete_attn_head(seq, out_sz, spa_adj, adj_type, activation, nb_nodes,
    ...
    with tf.name_scope('sp_hete_attn'):
        ...
        seq_fts_lists = [[] for _ in range(ent_types)]# 节点的特征，长度也为23
        ...
        for index in range(len(spa_adj)):
            ...
            # i,j = adj_type[2]
            f_1 = tf.layers.conv1d(seq_fts[j][j], 1, 1)#（1,34124，1）,第一类转第一类
            f_2 = tf.layers.conv1d(seq_fts[j][i], 1, 1)#（1,34124，1），第二类转第一类

            f_1 = tf.reshape(f_1, (-1, 1))#shape=(34124,1)
            f_2 = tf.reshape(f_2, (-1, 1))#shape=(34124,1)

            #根据sparse_adj_input对应的矩阵中的索引，在f_1、f_2中取值
            ...
            #验证注意力稀疏问题---------------
            # coefs = tf.SparseTensor(indices=spa_adj[index].indices, 
            #         values=spa_adj[index].values, 
            #         dense_shape=spa_adj[index].dense_shape)

            # #直接把权重与邻接矩阵相结合
            # f_1 = spa_adj[index] * f_1 #广播，adj_ij每一行乘以f_1对应行的值
            # f_2 = spa_adj[index] * tf.transpose(f_2, [1,0])#列
            ...
            coefs_lists[j].append(coefs)
            #转换的特征经过dropout，然后根coefs一样，添加到seq_fts_lists中(这一部分要单独处理)
            if in_drop != 0.0:
                seq_fts_ij = tf.nn.dropout(seq_fts[j][i], keep_prob=1-in_drop)
            seq_fts_lists[j].append(tf.squeeze(seq_fts_ij))

        #coefs_lists，9个元素，每个元素存放了相关adj下的注意力权重，形状是稀疏的adj，里面的连边值乘以注意力权重
        ...
        # seq_fts = [tf.reduce_sum(seq_fts_list, 0) for seq_fts_list in seq_fts_lists]
        #式5
        ...
        # ret = vals
        # #激活函数
        # ret1 = [activation(tf.nn.l2_normalize(r,dim=-1)) for r in vals]
        # ret1 = vals

        # return seq_fts,vals,coefs_lists,seq_fts_lists,coefs,vals,ret,ret1
        return vals
